    ...
    <div class="flex items-center overflow-none">
        <main class="flex justify-center items-center w-full">
            ...
            >
                ...
                <div
                    class="shadow-md rounded shadow-sky-950 border-2 border-sky-950 max-md:order-2"
                >
                ...
                <div
                    class="shadow-md rounded shadow-sky-950 border-2 border-sky-950 max-md:order-1"
                >
...
companies.value?.sort(
    (company1, company2) => Number(company1.order) - Number(company2.order)
);

const schools = ref(
    (
        await useAsyncData('schools', () => {
            return $directus.request(
                $readItems('schools', {
                    fields: ['*'],
                })
...
projects.value?.sort(
    (project1, project2) => Number(project1.order) - Number(project2.order)
);

const technologies = (
    await useAsyncData('technologies', () => {
        return $directus.request(
